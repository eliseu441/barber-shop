...
export class CalendarService {
  ...
  async createCalendar(createCalendarDto: CreateCalendarDto): Promise<Event> {
    ...
    const user = await this.userRepository.findOneBy({ id: createCalendarDto.userId });
    if (!user) {
      throw new Error(`User with id ${createCalendarDto.userId} not found`);
    }
  ...
  async findAllCalendar(): Promise<{ title: string; start: Date; end: Date; description: string; userId: number }[]> {
